...
with torch.no_grad():
    ...
    for index, sample in enumerate(bar):
        _label = sample['labels']['M'].clone().detach().to(device).float()
        Ts, Vs, As, Td, Vd, Ad = model(sample, None, mode='test', return_all_fea=True)
        x_t_simi1.append(Ts)
        x_v_simi1.append(Vs)
        x_a_simi1.append(As)
