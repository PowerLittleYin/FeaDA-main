...
class MOSIDataset(Dataset):
    def __init__(self, type, use_similarity=False, simi_return_mono=False, config=default_config):
        raw_data_path = config.MOSI.path.raw_data_path
...
def MOSIDataloader(type , batch_size=None, use_sampler=False, use_similarity=False, simi_return_mono=False,
                    shuffle=True,
                    num_workers=4,
                    prefetch_factor=2,
    ...
    if batch_size is None:
        ...
        return
    dataset = MOSIDataset(type, use_similarity=use_similarity, simi_return_mono=simi_return_mono)
    sampler = None
    drop_last = False
    if use_sampler:
        shuffle = False
