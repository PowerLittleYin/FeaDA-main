...
sys.path.append('..')
from MOSI.models.Vision_encoder import VisionPretrain
from MOSI.utils import write_log, set_random_seed
from MOSI.utils import write_config
...
def Vtrain(config, metrics, seed, train_data, valid_data):
    ...
    total_epoch = config.MOSI.downStream.visionPretrain.epoch

    lr = config.MOSI.downStream.visionPretrain.lr
    decay = config.MOSI.downStream.visionPretrain.decay
...
def Vtest(config, metrics, test_data, num=0):
    log_path = config.LOGPATH + "MOSI_VisionPretrain_Test." + \
