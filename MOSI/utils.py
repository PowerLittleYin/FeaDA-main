...
def write_config(config, log_path):
    d = {}
    d['path'] = {}
    for key in dir(config.MOSI.path):
        if key.startswith('__') or key.startswith('_'):
            continue
        d['path'][key] = getattr(config.MOSI.path, key)
    write_log(d, path=log_path)

    d = {}
    d['downStream'] = {}
    for key in dir(config.MOSI.downStream):
    ...
    d = {}
    d['TVAtrain'] = {}
    for key in dir(config.MOSI.downStream.TVAtrain):
        if key.startswith('__') or key.startswith('_'):
            continue
        d['TVAtrain'][key] = getattr(config.MOSI.downStream.TVAtrain, key)
    write_log(d, path=log_path)
...
class Metrics:
    ...
    def eval_mosi_regression(self, y_true, y_pred, exclude_zero=False):
