...
class PositionEncodingTraining(nn.Module):
    ...
    def __init__(self, fea_size=None, tf_hidden_dim=None, drop_out=None, config=cfg):
        super().__init__()
        if fea_size is None:
            fea_size = config.MOSEI.downStream.audio_fea_dim
        if tf_hidden_dim is None:
            tf_hidden_dim = config.MOSEI.downStream.encoder_fea_dim
        if drop_out is None:
            drop_out = config.MOSEI.downStream.audio_drop_out
...
class AudioEncoder(nn.Module):
    def __init__(self, config=cfg):
        ...
        self.audio_seq_len = config.MOSEI.downStream.audio_seq_len
        self.audio_nhead = config.MOSEI.downStream.audio_nhead
        self.dim_feedforward = config.MOSEI.downStream.encoder_fea_dim
        self.audio_tf_num_layers = config.MOSEI.downStream.audio_tf_num_layers
        self.attn_dropout = config.MOSEI.downStream.audio_drop_out
        self.attn_mask = config.MOSEI.downStream.audio_attn_mask
