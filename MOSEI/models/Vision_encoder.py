...
class PositionEncodingTraining(nn.Module):
    ...
    def __init__(self, config=cfg, fea_size=None, tf_hidden_dim=None, drop_out=None):
        super().__init__()
        if fea_size is None:
            fea_size = config.MOSEI.downStream.vision_fea_dim
        if tf_hidden_dim is None:
            tf_hidden_dim = config.MOSEI.downStream.encoder_fea_dim
        if drop_out is None:
            drop_out = config.MOSEI.downStream.vision_drop_out
...
class VisionEncoder(nn.Module):
    def __init__(self, config=cfg):
        ...
        self.dim_feedforward = config.MOSEI.downStream.encoder_fea_dim
