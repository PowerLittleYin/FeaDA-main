...
import torch
# from pytorch_metric_learning.piplosses.ntxent_loss import NTXentLoss
...
def write_config(config, log_path):
    d = {}
    d['path'] = {}
    for key in dir(config.MOSEI.path):
        if key.startswith('__') or key.startswith('_'):
            continue
        d['path'][key] = getattr(config.MOSEI.path, key)
    write_log(d, path=log_path)

    d = {}
    d['downStream'] = {}
    for key in dir(config.MOSEI.downStream):
        if key.startswith('__') or key.startswith('_') or key.endswith('train'):
            continue
        d['downStream'][key] = getattr(config.MOSEI.downStream, key)
    write_log(d, path=log_path)

    d = {}
    d['TVAtrain'] = {}
    for key in dir(config.MOSEI.downStream.TVAtrain):
        if key.startswith('__') or key.startswith('_'):
            continue
        d['TVAtrain'][key] = getattr(config.MOSEI.downStream.TVAtrain, key)
    write_log(d, path=log_path)

    d = {}
    d['audioPretrain'] = {}
    for key in dir(config.MOSEI.downStream.audioPretrain):
        if key.startswith('__') or key.startswith('_'):
            continue
        d['audioPretrain'][key] = getattr(config.MOSEI.downStream.audioPretrain, key)
    write_log(d, path=log_path)

    d = {}
    d['visionPretrain'] = {}
    for key in dir(config.MOSEI.downStream.visionPretrain):
        if key.startswith('__') or key.startswith('_'):
            continue
        d['visionPretrain'][key] = getattr(config.MOSEI.downStream.visionPretrain, key)
    write_log(d, path=log_path)
...
class cont_NTXentLoss(NTXentLoss):
    ...
    def compute_loss(self, embeddings, labels, indices_tuple=None, ref_emb=None, ref_labels=None):
        ...
        if all(len(x) <= 1 for x in indices_tuple):
            return self.zero_losses()
        mat = self.distance(embeddings)
        # return self.loss_method(mat, labels, indices_tuple)  # 确保参数传递正确
        return self.loss_method(mat, indices_tuple)  # 确保参数传递正确
    # def compute_loss(self, embeddings, labels, indices_tuple=None, ref_emb=None, ref_labels=None):
    #     indices_tuple = convert_to_pairs(indices_tuple, labels)
    #     if all(len(x) <= 1 for x in indices_tuple):
    #         return self.zero_losses()
