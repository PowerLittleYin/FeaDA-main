...
sys.path.append('..')
from MOSEI.models.Vision_encoder import VisionPretrain
from MOSEI.utils import write_log, set_random_seed
from MOSEI.utils import write_config
...
def Vtrain(config, metrics, seed, train_data, valid_data):
    ...
    total_epoch = config.MOSEI.downStream.visionPretrain.epoch

    lr = config.MOSEI.downStream.visionPretrain.lr
    decay = config.MOSEI.downStream.visionPretrain.decay
...
def Vtest(config, metrics, test_data, num=0):
    log_path = config.LOGPATH + "MOSEI_VisionPretrain_Test." + \
