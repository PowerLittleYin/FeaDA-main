...
class MOSEIDataset(Dataset):
    ...
    def __init__(self, type, use_similarity=False, simi_return_mono=False, config=default_config, load_cache=True):
        self.raw_data_path = config.MOSEI.path.raw_data_path
        ...
        if 'vision_lengths' not in self.data.keys():
            vision_len = self.data['vision'].shape[1]
            self.data['vision_lengths'] = [vision_len] * self.data['vision'].shape[0]
        self.cache_dir = config.MOSEI.path.cache_dir  # 添加缓存目录
        self.type = type
        self.use_similarity = use_similarity
        self.simi_return_mono = simi_return_mono
        ...
        self.__gen_mask()
        # 尝试加载缓存
        if self.load_cache and self.__load_cache():
            print(f"Loaded cache for {type} dataset")
        else:
            # 如果没有缓存或加载失败，则正常初始化
            if type == 'train' and use_similarity:
                self.__scale()
    ...
    def __save_cache(self):
        ...
        torch.save({
            'data': self.data,
            'scaled_audio': self.scaled_audio if hasattr(self, 'scaled_audio') else None,
            'scaled_vision': self.scaled_vision if hasattr(self, 'scaled_vision') else None,
            'cos_matrix_M': self.cos_matrix_M if hasattr(self, 'cos_matrix_M') else None,
            'rank_M': self.rank_M if hasattr(self, 'rank_M') else None,
            'M_retrieve': self.M_retrieve if hasattr(self, 'M_retrieve') else None
        }, cache_path, pickle_protocol=pickle.HIGHEST_PROTOCOL)
        print(f"Saved cache for {self.type} dataset to {cache_path}")
    ...
    def __load_cache(self):
        ...
        try:
            ...
            if 'scaled_audio' in cache_data and cache_data['scaled_audio'] is not None:
                self.scaled_audio = cache_data['scaled_audio']
            if 'scaled_vision' in cache_data and cache_data['scaled_vision'] is not None:
                self.scaled_vision = cache_data['scaled_vision']
            if 'cos_matrix_M' in cache_data and cache_data['cos_matrix_M'] is not None:
                self.cos_matrix_M = cache_data['cos_matrix_M']
            if 'rank_M' in cache_data and cache_data['rank_M'] is not None:
                self.rank_M = cache_data['rank_M']
            if 'M_retrieve' in cache_data and cache_data['M_retrieve'] is not None:
                self.M_retrieve = cache_data['M_retrieve']
            print(f"Loaded cache for {self.type} dataset from {cache_path}")
            return True
    ...
    def __get_cache_path(self):
        """获取缓存文件路径"""
        return os.path.join(self.cache_dir, f"{self.type}_dataset_cache.pth")
            ...
            for i in range(cut):
                ...
                for j in range(cut):
                    ...
                    self.cos_matrix_M[start_row:end_row, start_column:end_column] = cos(
                        ...
                        B[:, start_column:end_column, :].to(torch.device(default_config.DEVICE))).to(
                        torch.device("cpu"))
            del A, B
...
def MOSEIDataloader(type , batch_size=None, use_sampler=False, use_similarity=False, simi_return_mono=False,
                    shuffle=True,
                    num_workers=4,
                    prefetch_factor=2,
    ...
    if batch_size is None:
        ...
        return
    dataset = MOSEIDataset(type, use_similarity=use_similarity, simi_return_mono=simi_return_mono)
    sampler = None
    drop_last = False
    if use_sampler:
        shuffle = False
