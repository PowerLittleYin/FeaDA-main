...
import numpy as np
from SIMS.utils import cont_NTXentLoss
import os
import sys
...
class TVA_fusion(nn.Module):
    ...
    def forward(self, sample1, sample2, mode='train', device=cfg.DEVICE):
        ...
        if mode == 'train':
            ...
            loss = pred_loss + 0.02 * sup_const_loss + 0.03 * mono_task_loss + 0.0* loss_nce

            # #0.1 0.01 0.05 37.06
            # 0.05 0.01 0.05 35.8088
            # 0.05 0.01 0.01 33.5028
            # 0.01 0.01 0.01 33.8767
            # 0.02 0.01 0.01 33.43584

            #0.02 0.01 0.03 34.1088
            #0.02 0.01 0.0005 32.94012 *
            # 0.02 0.01 0.0003 33.02192
            #0.02 0.01 0.0 32.93012 *

            # 0.02 0.05 0.0 32.9292
            # 0.02 0.06 0.0 32.7864
            # 0.02 0.04 0.0 32.7653
            # 0.02 0.03 0.0 32.7613 *
            # 0.02 0.0 0.0 33.029
