...
class PositionEncodingTraining(nn.Module):
    ...
    def __init__(self, config=cfg, fea_size=None, tf_hidden_dim=None, drop_out=None):
        super().__init__()
        if fea_size is None:
...
class TfEncoder(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward, num_layers, dropout=0.2, activation='gelu', config=cfg
                 ):
...
class VisionEncoder(nn.Module):
    def __init__(self, config=cfg):
        ...
        self.vision_fea_dim = config.SIMS.downStream.vision_fea_dim
        self.vision_seq_len = config.SIMS.downStream.vision_seq_len

        self.encoder_fea_dim = config.SIMS.downStream.encoder_fea_dim
        self.vision_nhead = config.SIMS.downStream.vision_nhead
        self.vision_tf_num_layers = config.SIMS.downStream.vision_tf_num_layers
        self.attn_dropout = config.SIMS.downStream.vision_drop_out
        self.attn_mask = config.SIMS.downStream.vision_attn_mask
        ...
        self.encoder = TfEncoder(d_model=self.encoder_fea_dim, nhead=self.vision_nhead,
                                 dim_feedforward=self.dim_feedforward,
                                 num_layers=self.vision_tf_num_layers,
